...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=cosmixMantle \
...
DOCKER_IMAGE_NAME = Cosmixone/cosmixMantle
DOCKER_TAG_NAME = latest
DOCKER_CONTAINER_NAME = cosmixMantle-container
DOCKER_CMD ?= "/bin/sh"
...
#
# Run CosmixCore on docker
# Example Usage:
# 	make docker-build   ## Builds CosmixCore binary in 2 stages, 1st builder 2nd Runner
# 						   Final image only has the compiled CosmixCore binary
# 	make docker-interactive   ## Will start an shell session into the docker container
# 								 Access to CosmixCore binary here
